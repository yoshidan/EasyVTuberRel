...
namespace App.Main.Scripts.FaceTracking
{
    ...
    public class FaceParts
    {
        ...
        public MouthOpenPart MouthOpen { get; }
        ...
        public class MouthOpenPart
        {
            public float mouthOpenYRatio = 0;

            public float mouthOpenXRatio = 0;
            
            public void Update(Vector2[] landmarks)
            {
                var distanceOfNoseHeight = new Vector2(landmarks[33].x - (landmarks[39].x + landmarks[42].x / 2), landmarks[33].y - (landmarks[39].y + landmarks[42].y) / 2)
                    .sqrMagnitude;
                var distanceOfMouthWidth = new Vector2(landmarks[48].x - landmarks[54].x, landmarks[48].y - landmarks[54].y)
                    .sqrMagnitude;
                var distanceOfMouthHeight = new Vector2(landmarks[51].x - landmarks[57].x, landmarks[51].y - landmarks[57].y)
                    .sqrMagnitude;
                var distanceBetweenEyes = new Vector2(landmarks[39].x - landmarks[42].x, landmarks[39].y - landmarks[42].y)
                    .sqrMagnitude;

                mouthOpenYRatio = Mathf.InverseLerp(0.06f, .6f, distanceOfMouthHeight / distanceOfNoseHeight);
                mouthOpenXRatio = Mathf.InverseLerp(1.8f, 2.0f, distanceOfMouthWidth / distanceBetweenEyes);
            }
