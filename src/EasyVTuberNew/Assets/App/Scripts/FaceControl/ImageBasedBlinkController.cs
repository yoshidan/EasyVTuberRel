...
namespace App.Main.Scripts.FaceControl
{
    ...
    public class ImageBasedBlinkController : MonoBehaviour
    {
        ...
        public void Apply(VRMBlendShapeProxy proxy)
        {
            ...
            float left = _currentLeftBlink > closeThreshold ? 1.0f : 0.0f;
            float right = _currentLeftBlink > closeThreshold ? 1.0f : 0.0f;
            proxy.ImmediatelySetValue(BlinkLKey, left);
            proxy.ImmediatelySetValue(BlinkRKey, right);
        }
